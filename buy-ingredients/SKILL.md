...
2. 确认邮编（可选）：
   ...
   - 如用户未提供 → 不传 `--postcode`，仅搜索商品不查门店库存
   - 后续使用 → 从记忆读取邮编，向用户确认是否仍用该邮编
3. 将中文食材名翻译为英文，并参照 `references/marksandspencer/food-catalogue.md` 确定每种食材对应的完整路径（必须走到目录树的最底层叶子节点）
4. 运行脚本（在 `scripts/` 目录下执行）：
   ```
...
5. 将脚本输出的结果打印给用户
6. 将查询到的商品记录追加到 `.recipes/bills.csv`：
