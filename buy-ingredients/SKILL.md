...
6. 将脚本输出的结果打印给用户
7. 将购物清单追加到对应菜谱文件 `.recipes/{菜名}.md` 末尾，格式为 `## Shopping List` 表格（商品、价格、库存）
   - 如果菜谱中已有 `## Shopping List` 段落，替换为最新结果
   - 仅在食材来源为菜谱时执行，用户直接指定食材时跳过
8. 将查询到的商品记录追加到 `.recipes/bills.csv`：
   - 如果文件不存在，先写入表头：`日期,商品,价格`
   - 每条商品一行：`{当天日期},{商品名},{价格}`
   - 日期格式 `YYYY-MM-DD`，商品名使用 M&S 原文（英文），价格为纯数字（单位 £）
   - 如果商品无价格信息（如缺货），价格留空
